...
let invLength = inventory.length - 1;
...
//console.log(`Car 33 is a ${car33.car_year} ${car33.car_make} ${car33_car_model}`);

// ==== Challenge 2 ====
// The dealer needs the information on the last car in their inventory.  What is the make and model of the last car in the inventory?  Log the make and model into the console.
let lastCar = 0;
let lastCarId = inventory.length - 1;
//console.log(`${inventory[lastCarId].car_make}  ${inventory[lastCarId].car_model}`);}
...
let oldCars = [];
let oldCarsList = carYears.filter(carYears => carYears < 2000);
//console.log (oldCarsList);

// ==== Challenge 6 ====
// A buyer is interested in seeing only BMW and Audi cars within the inventory.  Return an array that only contains BMW and Audi cars.  Once you have populated the BMWAndAudi array, use JSON.stringify() to show the results of the array in the console.
let carMake =  [];
for (let i = 0; i < inventory.length; i++) carMake.push(inventory[i].car_make);
let BMWAndAudi = carMake.filter(word => word === "BMW" || word === "Audi" )
